...
# Build mesh for 3D model (reconsidered surface: curve as parting line, clamshell loft in both directions with curvature continuity, curve-driven profile for caps)
def build_mesh(x_curve, y_curve, height=0.5, num_rings=20, num_points=None):
    ...
    with a curve-driven profile (s**3 for G2 continuity at the parting line) to form a clamshell model. The caps are 
    radial lofts considerate of orthographic projections for smooth appearance in XY, XZ, YZ views.
    
    Args:
        x_curve (array): X coordinates of the closed curve (parting line).
        y_curve (array): Y coordinates of the closed curve (parting line).
        height (float): Total height of the model (split symmetrically).
        num_rings (int): Number of radial rings for the loft (including boundary).
        num_points (int, optional): Number of points to sample from the curve.
    
    Returns:
        vertices (np.array): 3D vertices of the mesh.
        faces (list): List of triangular faces.
    """
    ...
    for l in range(1, num_rings):
        s = l / (num_rings - 1.0)
        scale = 1 - s
        g_val = (height / 2) * s**3  # Curve-driven profile for G2 continuity (flat at s=0)
        base = len(vertices)
    ...
    # Lower loft: inward rings and center (mirrored profile)
    ...
    # Faces for upper loft (quads between rings, fan to center)
    ...
    for i in range(n):
        next_i = (i + 1) % n
        faces.append([center_upper, base + next_i, base + i])  # Reversed for outward normal if needed
    
    # Faces for lower loft (quads between rings, note order for normals)
    ...
    for i in range(n):
        next_i = (i + 1) % n
        faces.append([center_lower, base + i, base + next_i])  # Adjusted for outward normal
